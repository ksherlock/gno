...
static const char *
#ifdef USE_PROTOS
getnum(register const char *strp, int * const nump, const int min,
	const int max)
#else
...
static int
#ifdef USE_PROTOS
tzparse(const char *name, register struct state * const sp, const int lastditch)
#else
...
timesub(const time_t * const timep, const long offset,
	register const struct state * const sp, register struct tm * const tmp)
#else
